...
### 6.2 多分类拓展：One-vs-Rest (OvR) 与 Softmax 回归
逻辑回归原生只支持二分类。当面对多分类任务（如识别 0~9 十种手写数字）时，通常有两种拓展方式：

1. **One-vs-Rest (OvR / 一对多)**：为 $K$ 个类别分别训练 $K$ 个独立二分类器，哪个分类器输出的概率最高就属于哪个类；
2. **Softmax 回归 (Multinomial Logistic Regression)**：将 Sigmoid 扩展为 Softmax 函数，直接归一化输出 $K$ 个类别的概率分布：
