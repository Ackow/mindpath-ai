...
order: 1
difficulty: beginner
prerequisites: [ml-workflow-primer]
relatedNotes: []
...
3. **计算对权重 $w$ 与偏置 $b$ 的梯度**：
   - $\frac{\partial L}{\partial w} = \frac{2}{5} [(-0.2) \times 1 + (-0.3) \times 2 + (-1.1) \times 3 + (-2.0) \times 4 + (-1.9) \times 5] = 0.4 \times (-21.6) = -8.64$
   - $\frac{\partial L}{\partial b} = \frac{2}{5} [-0.2 - 0.3 - 1.1 - 2.0 - 1.9] = 0.4 \times (-5.5) = -2.20$
4. **进行一次梯度更新（迈出下山一步）**：
   - $w_{\text{new}} = 1.5 - 0.01 \times (-8.64) = 1.5864$
   - $b_{\text{new}} = 0.5 - 0.01 \times (-2.20) = 0.5220$
...
$$w_{new} = w_{old} - \alpha \cdot \nabla_w L = w_{old} - \alpha \cdot \frac{2}{N} X^T (\hat{Y} - Y)$$
...
接下来，请前往下个模块，探索分类问题的基石：

- **下一章节**：[02. 逻辑回归与二分类模型](/learn/supervised-learning/02-logistic-regression)
