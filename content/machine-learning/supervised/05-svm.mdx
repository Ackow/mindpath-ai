...
difficulty: advanced
elective: true
studyNote: 进阶选学：建议先完成线性模型、kNN/朴素贝叶斯和模型评估；初学者先理解最大间隔与调用方式，拉格朗日对偶、KKT 条件和核技巧推导可随后学习。
...
nextNotes: [supervised-ensemble-learning-random-forest]
map:
...
6. **识别工程踩坑点**：掌握 SVM 对特征缩放的极高敏感度以及大数据集下 $O(N^2 \sim N^3)$ 时间复杂度的应对方案。
...
| **大训练集卡死** | `SVC.fit()` 运行数小时不出结果 | `SVC` 基于 LibSVM 求解器，计算复杂度为 $O(N^2 \sim N^3)$，当样本数 $N > 50000$ 时极其缓慢 | 改用线性近似的 `LinearSVC` 或 `SGDClassifier(loss='hinge')` 降低时间复杂度 |
...
- **下一章推荐**：学习 **[06. 随机森林与集成学习](/learn/machine-learning/supervised/06-ensemble-learning-random-forest)**，探索可解释性强、天然支持非线性特征的树形模型！
- **相关前置与扩展阅读**：
