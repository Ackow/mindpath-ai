---
id: py-visualization
title: 02. Matplotlib 与 Seaborn 数据可视化
...
2. **训练过程诊断 (Training Diagnostics)**：绘制训练集与验证集的损失曲线 (Loss Curves) 和准确率变化，快速诊断模型状态。
3. **模型评估与误差分析 (Error Analysis)**：通过热力图直观定位分类预测结果。
...
### 3.4 混淆矩阵 (Confusion Matrix) 热力图

<ConceptCard title="💡 后续机器学习预告：混淆矩阵 (Confusion Matrix)" type="tip">
在后续【阶段 1 机器学习】分类算法中，我们将使用混淆矩阵评估模型的预测对错。在 Python 阶段，我们重点掌握如何使用 `sns.heatmap()` 将二维矩阵转化为直观的热力图！
</ConceptCard>

![混淆矩阵热力图示例](/assets/python/confusion_matrix.png)

<RunnableCodeBlock title="实战：混淆矩阵 Confusion Matrix 热力图绘制">
{`import matplotlib.pyplot as plt
import numpy as np

# 1. 模拟 3 分类模型的混淆矩阵 (True Labels vs Predicted Labels)
conf_matrix = np.array([
    [45,  3,  2],   # 类别 0 (Setosa) 真实样本 50 个
    [ 4, 42,  4],   # 类别 1 (Versicolor) 真实样本 50 个
    [ 1,  6, 43]    # 类别 2 (Virginica) 真实样本 50 个
])
...
# 2. 使用 Matplotlib 原生 imshow 绘制渐变热力图
im = ax.imshow(conf_matrix, cmap='Blues')

# 3. 添加右侧颜色数值标尺 (Colorbar)
...
ax.set_title("Confusion Matrix Heatmap (Matplotlib)", fontsize=12, fontweight="bold")
ax.set_xlabel("Predicted Label", fontsize=10)
ax.set_ylabel("True Label", fontsize=10)

plt.tight_layout()
plt.show()
print("混淆矩阵热力图绘制完毕：主对角线 45, 42, 43 为预测正确样本。")`}
</RunnableCodeBlock>

#### 本地环境扩展：Seaborn 混淆矩阵官方标准写法 (`sns.heatmap`)

在本地 IDE、Jupyter Notebook 或服务器 Python 环境中，推荐直接使用 Seaborn 官方极其优雅的 `sns.heatmap()` 实现混淆矩阵可视化：
...
ax.set_title("Confusion Matrix Heatmap (Seaborn Standard)", fontsize=12, fontweight="bold")
ax.set_xlabel("Predicted Label", fontsize=10)
ax.set_ylabel("True Label", fontsize=10)

plt.tight_layout()
plt.show()
print("Seaborn 混淆矩阵绘制完毕：主对角线数值越高代表预测越准确。")
```

### 2.5 多特征散点矩阵可视化 (Pandas scatter_matrix)

当数据集包含 3~6 个连续数值特征时，手动逐个画散点图效率极低。Pandas 内置的 **`pd.plotting.scatter_matrix()`** 能够一键生成特征矩阵对角线（单特征分布/KDE 密度）与非对角线（两两特征相关性散点图）的全景图，且完全基于 Matplotlib 原生渲染，在浏览器端可 100% 顺畅执行！

<RunnableCodeBlock title="实战：Pandas scatter_matrix 多特征两两散点矩阵">
{`import pandas as pd
import numpy as np
import matplotlib.pyplot as plt

# 1. 构建示例 DataFrame (包含 3 个连续特征)
np.random.seed(0)
n = 100
...
# 2. 使用 Pandas 内置的 scatter_matrix 绘制全景图
# diagonal='hist' 使用原生直方图呈现对角线分布 (无需 scipy 扩展包，100% 兼容浏览器)
pd.plotting.scatter_matrix(
    df_demo, 
    diagonal='hist',   # 对角线呈现单特征直方图 (无需 scipy)
    color='#0d9488',   # 散点颜色 (Teal)
    alpha=0.7, 
    figsize=(7, 7)
)

plt.suptitle("Multi-Feature Pairwise Distribution (Pandas Scatter Matrix)", y=0.93, fontsize=11, fontweight='bold')
plt.tight_layout()
plt.show()
print("散点矩阵绘制完毕：对角线呈现单特征直方图分布，非对角线呈现两两相关性。")`}
</RunnableCodeBlock>

<ConceptCard title="本地 Python 离线环境最佳实践：Seaborn & Pearson 相关系数矩阵" type="note">
在本地 IDE、Jupyter Notebook 或服务器 Python 环境中（已安装 `seaborn` 和 `scipy`），可以直接使用工业级的高阶可视化 API：
</ConceptCard>

#### 1. 本地 Seaborn 混淆矩阵与 Pearson 相关系数矩阵热力图
...
# --- A. Pearson 皮尔逊相关系数矩阵 (Pearson Correlation Matrix) ---
np.random.seed(42)
df_features = pd.DataFrame({
    'Petal_Length': np.random.normal(5, 1, 100),
    'Petal_Width': np.random.normal(2, 0.5, 100),
    'Sepal_Length': np.random.normal(6, 1, 100),
    'Sepal_Width': np.random.normal(3, 0.5, 100)
})

# 计算特征间两两 Pearson 相关系数矩阵 (-1 ~ +1)
pearson_corr = df_features.corr(method='pearson')

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))

# 绘制 Pearson 相关系数热力图 (使用极性发散色盘)
sns.heatmap(
    pearson_corr, 
    annot=True,         # 显示皮尔逊相关系数值 (r)
    fmt='.2f',          # 保留 2 位小数
    cmap='vlag',        # 极性发散色盘 (正相关红/负相关蓝)
    vmin=-1, vmax=1,
    square=True,
    cbar_kws={'label': 'Pearson Correlation (r)'},
    ax=ax1
)
ax1.set_title("Pearson Correlation Matrix Heatmap", fontsize=12, fontweight='bold')

# --- B. Seaborn 3 分类混淆矩阵热力图 ---
conf_matrix = np.array([
    [45,  3,  2],
    [ 4, 42,  4],
    [ 1,  6, 43]
...
ax2.set_title("Confusion Matrix Heatmap (Seaborn)", fontsize=12, fontweight='bold')
...
#### 2. 本地 Seaborn Pairplot 多特征带类别分类全景图
...
df_iris = pd.DataFrame({
    'Petal_Length': np.random.normal(5, 1.5, n),
    'Petal_Width': np.random.normal(2, 0.5, n),
    'Sepal_Length': np.random.normal(6, 1.2, n),
    'Species': np.random.choice(['Setosa', 'Versicolor'], size=n)
})

# 本地 Seaborn 一键生成对角线 KDE 密度 + 非对角线按类别着色的散点全景图
grid = sns.pairplot(
    df_iris, 
    hue='Species',      # 按花卉类别颜色区分
    palette='Set1',     # 经典高对比调色盘
    corner=True,        # 隐藏右上角重复的对称三角区
    diag_kind='kde'     # 对角线使用核密度估计 (KDE)
)
grid.fig.suptitle("Seaborn Pairplot Feature Distributions", y=1.02, fontsize=12, fontweight='bold')
plt.show()
```
