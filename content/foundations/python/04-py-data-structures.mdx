...
1. 为什么训练历史适合用列表，学习率配置适合用字典？
2. `scores[1:4]` 包含哪些位置？为什么切片通常使用半开区间？
3. `config["seed"]` 和 `config.get("seed", 42)` 在键不存在时有什么区别？
4. 为什么集合不能通过索引访问？如何在去重后保留第一次出现的顺序？
5. `alias = original`、`original.copy()` 和 `deepcopy(original)` 对嵌套列表分别意味着什么？
