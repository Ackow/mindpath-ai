...
1. **概念理解**：如果创建了一个名为 `math.py` 的文件，并在其中运行 `import math; print(math.sqrt(4))`，会发生什么？为什么？
2. **代码阅读**：阅读以下代码，写出输出结果：
   ```python
   class A:
       def show(self):
           return "Class A"
   ...
   print(B().show())
   ```
3. **实战练习**：编写一个 `ModelConfig` 的 `@dataclass`，包含 `learning_rate`（默认 `0.01`）、`hidden_dim`（默认 `128`）和 `use_gpu`（默认 `True`），并编写一个函数接收该配置并打印调试信息。

---

## 下一步与参考资料

- **下一步**：学习 [07.文件、异常、调试与测试](/learn/foundations/python/07-py-files-debugging)，掌握 AI 数据文件的安全读写与训练异常定位。
- **参考资料**：
