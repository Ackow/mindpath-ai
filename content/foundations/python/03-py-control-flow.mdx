...
下面这张流程图把本章最常见的执行路径放在一起：条件分支只选择一条路径，循环则在每轮结束后回到循环头，直到自然结束或遇到 `break`。
...
阅读代码时，可以沿着箭头逐步追踪：每个菱形只回答一个问题，每条箭头都应该能解释“下一行会执行什么”。
...
这个动画只演示本节最容易混淆的 `if / elif / else`，不混入循环。高亮流程图节点代表当前执行位置；可以手动输入“分数”和“及格阈值”，再点击播放或逐步按钮：分数为 95 会走 `if` 的“优秀”路径，72 会走 `elif` 的“及格”路径，低于阈值则走 `else`。切换到下一步时，只有刚刚走过的那条箭头会出现流动效果；每个节点还会显示正在执行的代码和它对控制流的影响。
...
5. `[x for x in values if x > 0]` 与 `[x if x > 0 else 0 for x in values]` 的结果为何不同？
6. 什么时候应该放弃推导式，改用普通循环？
