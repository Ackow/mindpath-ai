...
symbols:
  - {symbol: "X \\in \\mathbb{R}^{n \\times d}", mean: "样本特征矩阵，n 行样本，d 个特征"}
  - {symbol: "y \\in \\{0, 1, 2\\}^n", mean: "类别标签向量，分别代表 Setosa、Versicolor、Virginica"}
  - {symbol: "\\text{Accuracy}", mean: "准确率，预测正确的样本数占总样本数的比例"}
  - {symbol: "\\text{Precision}", mean: "精确率，被模型预测为某一类别的样本中实际属于该类别的比例"}
  - {symbol: "\\text{Recall}", mean: "召回率，实际属于某一类别的样本中被模型正确预测出来的比例"}
  - {symbol: "F_1", mean: "F1 分数，精确率与召回率的调和平均值"}
  - {symbol: "\\mathbf{C}_{i, j}", mean: "混淆矩阵中实际类别为 i 且被预测为类别 j 的样本数量"}
---
