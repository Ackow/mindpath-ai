...
<InteractiveQuiz
  title="8. NumPy 数值计算与向量化 课后互动自测"
  questions={[
    {
      id: "q1",
      type: "single",
      question: "已知数组 A 的 Shape 为 (3, 1)，数组 B 的 Shape 为 (3,)。执行 A + B 会发生什么？",
      options: [
        "A. 触发广播机制相加，最终结果的 Shape 为 (3, 3)",
        "B. 触发广播机制相加，最终结果的 Shape 为 (3, 1)",
        "C. 抛出 ValueError 表示维度无法匹配",
        "D. 触发按元素直接相加，Shape 为 (3,)"
      ],
      answer: 0,
      explanation: "广播机制规则：B (3,) 后向对齐补充维度变成 (1, 3)。比较 (3, 1) 与 (1, 3)，第一维 1 扩充为 3，第二维 1 扩充为 3，最终广播相加结果 Shape 为 (3, 3)。"
    },
    {
      id: "q2",
      type: "single",
      question: "若 Batch 特征矩阵 X 维度为 (64, 784)，线性层权重 W 维度为 (784, 10)，偏置 b 维度为 (10,)。计算 Z = X @ W + b 后，Z 的 Shape 是多少？",
      options: [
        "A. (64, 784)",
        "B. (64, 10)",
        "C. (784, 10)",
      ...
      answer: 1,
      explanation: "矩阵乘法 X @ W 维度为 (64, 784) @ (784, 10) -> (64, 10)；加上偏置 b (10,) 广播后保持 (64, 10)，对应 64 个样本在 10 个类别上的得分。"
    },
    {
      id: "q3",
      type: "multiple",
      question: "关于 NumPy 性能与内存视图 (View) 与 拷贝 (Copy)，以下哪些说法是正确的？（多选）",
      options: [
        "A. NumPy 向量化操作比标准 Python 纯循环快数十倍至上百倍",
        "B. 切片操作 arr[1:5] 返回的是原数组的视图 (View)，修改切片会同步修改原数组",
        "C. 花式索引 arr[[1, 3, 4]] 和布尔索引 arr[arr > 0] 返回的是新数组拷贝 (Copy)",
        "D. np.reshape(-1, 1) 会修改原数组的内在物理存储"
      ],
      answer: [0, 1, 2],
      explanation: "选项 A、B、C 均为 NumPy 核心视图与切片机制；选项 D 错误，reshape 只要物理内存连续通常返回视图，不会修改底层数据物理存储。"
    }
  ]}
/>
...
下一章：[09.pandas 数据处理与特征清洗](/learn/foundations/python/09-py-pandas)。将 NumPy 数组技能应用到表格特征清洗流中。
