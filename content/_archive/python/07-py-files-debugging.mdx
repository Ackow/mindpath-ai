...
<InteractiveQuiz
  title="07. 文件、异常、调试与测试 课后互动自测"
  questions={[
    {
      id: "q1",
      type: "code-output",
      question: "查看以下 Python 文件读取代码。在 Windows 系统上处理 UTF-8 编码的中文文本时，原代码最容易引发什么错误？应该如何修复？",
      codeSnippet: "# 原始代码\nf = open('data/chinese_text.txt', 'r')\ncontent = f.read()",
      options: [
        "A. 容易触发 UnicodeDecodeError；应改用 with open('data/chinese_text.txt', 'r', encoding='utf-8') as f:",
        "B. 容易触发 SyntaxError；应该用 load() 函数取代 open()",
        "C. 容易触发 OverflowError；应该增大内存缓冲区",
        "D. 原代码完全没有任何隐蔽隐患"
      ],
      answer: 0,
      explanation: "Windows 下 open() 默认使用系统 ANSI/GBK 编码，读取 UTF-8 中文文本会抛出 UnicodeDecodeError。应该始终使用 with 语句配合显式指定 encoding='utf-8'。"
    },
    {
      id: "q2",
      type: "single",
      question: "在 Batch 数据批处理或训练集加载过程中，为什么建议使用 try-except 捕获单条脏数据的处理，而不是让程序直接崩溃？",
      options: [
        "A. 捕获脏数据并记录日志可防止训练中断，避免因为 1% 的损坏样本导致数小时的训练前功尽弃",
        "B. try-except 可以自动将坏数据修复为正确数据",
        "C. 使用 try-except 能提升 GPU 计算速度",
        "D. Python 要求每个 for 循环内部必须包含 try-except"
      ],
      answer: 0,
      explanation: "海量真实数据中难免存在空值、坏图或破损文本。使用 try-except 容错隔离（记录脏数据 index 后 continue）可确保整体训练流程稳健进行。"
    },
    {
      id: "q3",
      type: "multiple",
      question: "关于 pathlib.Path 模块与文件操作，以下哪些说法是正确的？（多选）",
      options: [
        "A. Path 能够自动根据当前运行的操作系统（Windows/Linux/Mac）处理斜杠与反斜杠路径分隔符",
        "B. 使用 with open(...) 上下文管理器可以保证在离开代码块时自动关闭文件句柄，即使发生异常也不例外",
        "C. pytest 测试框架中，测试函数名通常需要以 test_ 开头",
        "D. 将 API 密钥直接硬编码写入代码文件并提交到 Git 仓库是推荐的做法"
      ],
      answer: [0, 1, 2],
      explanation: "选项 A、B、C 均为文件路径与自动化测试的标准规范；选项 D 极其危险，敏感密钥应存储于环境变量或 .env 文件，并将其加入 .gitignore 防止泄露。"
    }
  ]}
/>
...
下一章：[08.NumPy 数值计算](/learn/foundations/python/08-py-numpy)。开启 AI 矩阵与数值计算大门。
