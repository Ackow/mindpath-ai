...
<InteractiveQuiz
  title="02. 变量、基本类型与表达式 课后互动自测"
  questions={[
    {
      id: "q1",
      type: "single",
      question: "关于 Python 语言的类型系统特性，以下哪一项说法是准确的？",
      options: [
        "A. 动态类型意味着变量在运行时绑定类型，强类型意味着不会隐式将 \"3\" + 2 自动转换计算",
        "B. Python 是静态类型且弱类型语言",
        "C. 动态类型意味着类型不可更改，弱类型意味着可以隐式将字符串自动转为浮点数",
        "D. 只要使用 type() 判断，就能完全支持面向对象继承关系的子类检查"
      ],
      answer: 0,
      explanation: "Python 是动态强类型语言。动态指的是变量在运行时绑定类型，强类型指的是不支持不兼容类型的隐式自动转换（如字符串与整数相加会抛出 TypeError，需显式转换）。"
    },
    {
      id: "q2",
      type: "single",
      question: "在 Python 中，表达式 bool(\"False\") 的运行结果是什么？为什么？",
      options: [
        "A. False，因为字符串内容为 \"False\"",
        "B. True，因为 bool() 对非空字符串求值一律返回 True",
        "C. 抛出 ValueError 异常",
        "D. None"
      ],
      answer: 1,
      explanation: "Python 中 bool(s) 是基于真值测试（Truth Value Testing）。任何非空字符串（即使内容是 \"False\" 或 \"0\"）转为布尔值结果都是 True。只有空字符串 \"\" 结果才是 False。"
    },
    {
      id: "q3",
      type: "single",
      question: "在比较变量时，== 运算符与 is 运算符有什么本质区别？为什么判断 None 必须用 is？",
      options: [
        "A. == 比较内存地址，is 比较对象存储的值",
        "B. == 比较两对象的值是否相等，is 比较两对象的内存身份 (Identity)；None 是单例对象",
        "C. 两者完全没有任何区别，可以互换使用",
        "D. is 运行效率比 == 慢得多"
      ],
      answer: 1,
      explanation: "== 触发 __eq__ 比较值，is 比较对象的 id() 是否指向同一个内存地址。None 在 CPython 中是全局唯一的单例对象，使用 x is None 比 x == None 更安全且执行效率更高。"
    },
    {
      id: "q4",
      type: "code-output",
      question: "查看以下列表赋值与修改代码。运行后 print(features) 打印的最终结果是什么？",
      codeSnippet: "features = [1.0, 2.0]\nsame_features = features\nsame_features.append(3.0)\nprint(features)",
      options: [
        "A. [1.0, 2.0]",
        "B. [1.0, 2.0, 3.0]",
        "C. [3.0]",
      ...
      answer: 1,
      explanation: "列表是可变对象。same_features = features 仅仅复制了内存引用，两个变量指向同一个列表。修改 same_features 也会直接影响 features。如果需要独立副本，应使用 features.copy()。"
    }
  ]}
/>
...
下一章：[03.控制流与推导式](/learn/foundations/python/03-py-control-flow)。你将使用本章的布尔表达式、字符串和数值，遍历样本并构造批处理逻辑。
