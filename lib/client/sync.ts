...
export async function syncActivity(date: string, count: number, minutes: number) {
  try { await fetch('/api/activity', { method: 'PUT', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ date, count, minutes }) }); } catch { /* retry on the next activity */ }
}
